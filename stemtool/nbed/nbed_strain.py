...
def log_sobel4D(data4D, scan_dims, med_factor=30, gauss_val=3):
    ...
    if sum_dims < 2:
        data4D = np.transpose(data4D, (2, 3, 0, 1))
    data_lsb = np.zeros_like(data4D, dtype=float)
    for jj in range(data4D.shape[int(scan_dims[1])]):
        for ii in range(data4D.shape[int(scan_dims[0])]):
...
def get_inside(edges, cutoff=0.95):
    ...
    yy_aa2 = yy[aa2]
    xx_aa2 = xx[aa2]
    ang_range1 = np.zeros_like(yy, dtype=float)
    ang_range2 = np.zeros_like(yy, dtype=float)
...
def strain4D_general(
    ...
    LSB_ROI = np.zeros_like(ROI_4D, dtype=float)
    ...
    prominences = np.zeros((no_of_disks, merged_peaks.shape[0]), dtype=float)
...
def strain_figure(
    ...
    def ROI_RdBu_map(valmap, roi, valrange):
        plot_col = np.zeros((256, 3), dtype=float)
        for ii in range(255):
